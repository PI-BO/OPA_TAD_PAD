...
with open('result/exp_rush_hour.pickle', 'rb') as f:
...
plt.title('Roush hour occupancy example', fontsize=18)
plt.legend()
plt.show()
...
# rush hour(presanitized)
with open('result/exp_rush_hour_presanitized.pickle', 'rb') as f:
    anonymity_levels, losses_best, losses_generic, losses_linear, \
    losses_deep, distances_lm, distances_dm, distances_gt = pickle.load(f)
...
plt.xlabel('Anonymity level', fontsize=18)
plt.ylabel('Information loss', fontsize=18)
plt.title('Rush hour occupancy example', fontsize=18)
plt.legend()
plt.show()
