...
def main():
    ...
    resultDir = "./result"
    timestampStr = dt.datetime.now().strftime("%Y%m%d_%H%M%S")
    k_init = 10
    batch_size = 20
    mc_num = 5
    ...
    try:
        fh = logging.FileHandler(logDir + "/" + __file__[0:-3] + "_" + timestampStr + ".log")
    except:
        logger.error("Cannot create file: %s/%s_%s.log"% (logDir, __file__[0:-3], timestampStr))
    else:
    ...
    # option for displayed rows and columns
